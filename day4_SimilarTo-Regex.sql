...
Regex : Herhangi bir kod, metin içerisinde istenen yazı veya kod parçasının aranıp bulunmasını 
sağlayan kendine ait bir söz dizimi olan bir yapıdır.MySql de(Regexp_like) olarak kullanılır
...
harf_sayisi int
   ...
   --regex ile
   select * from kelimeler where kelime ~* 'ot' or kelime ~* 'at'
      select * from kelimeler where kelime ~*'[oa]t(.*)'
...
--select * from kelimeler where kelime like 'h%' and kelime like '%t' and kelime like '_[a-e]'


--İlk karakteri 's', 'a' veya 'y' olan "kelime" değerlerini çağırın.

select * from kelimeler where kelime like '^[s,a,y]%' -- ^ başlangıçı temsil eder
